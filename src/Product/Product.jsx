...
export  const Product = (props) =>  {
  ...
  <div className={style.contant_product} id={props.product.id}>
    ...
    <div className={style.link}> <a href="#" className={style.item_link}>{props.product.description}</a> </div>
    <p className={style.container_text}> Цена <strong className={style.price_one} id={"v"}> {props.product.price}</strong> руб</p>
    <p className={style.item_link_text}>КОЛИЧЕСТВО</p>
    <div id="{1000}">
      <button className={style.increase} id='inc{0}'> + </button>
      <input type={style.number} value={0}  className={style.input} readOnly></input>
